...
def check_and_install_dependencies(system_type, features=None):
    """
    ...
    if 'python' in required:
        for package in required['python']:
            if not is_package_installed(package):
                ...
                if not install_package_pip(package):
                    success = False
    
    # Check and install system packages if user has admin privileges
    if 'system' in required and os.geteuid() == 0:  # Only attempt if running as root
        for package in required['system']:
            if not check_system_dependency(package):
                logging.info(f"System package {package} not found, attempting to install...")
                if not install_system_package(package):
                    success = False
    elif 'system' in required:
        logging.warning("Cannot install system packages without admin privileges")
        logging.info("Required system packages: " + ", ".join(required['system']))
        success = False
